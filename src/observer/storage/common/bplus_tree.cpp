...
{  // 简化
  ...
  switch (attr_type) {
    ...
    case DATES: {
      Date d1 = *(Date *)first;
      Date d2 = *(Date *)second;
      // Assuming Date struct has member variables like year, month, day
      if (d1.year != d2.year)
    ...
    case EQUAL_TO:
      switch (attr_type) {
        ...
        case DATES: {
          ...
          Date d2 = *(Date *)value_;
          // Assuming Date struct has member variables like year, month, day
          flag = (d1.year == d2.year) && (d1.month == d2.month) && (d1.day == d2.day);
        } break;
        default:
          LOG_PANIC("Unknown attr type: %d", attr_type);
      }
    ...
    case LESS_THAN:
      switch (attr_type) {
        ...
        case DATES: {
          ...
          Date d2 = *(Date *)value_;
          // Assuming Date struct has member variables like year, month, day
          flag = (d1.year < d2.year) || ((d1.year == d2.year) && (d1.month < d2.month)) || ((d1.year == d2.year) && (d1.month == d2.month) && (d1.day < d2.day));
        } break;
        default:
          LOG_PANIC("Unknown attr type: %d", attr_type);
      }
